import { sql, loadConfig } from '../database/connection';
...
export class AIReviewService {
  /**
   * Main entry point to extract reviews for an item.
   * Runs in the background (fire-and-forget).
   */
  static async extractAndSaveReviews(itemId: string, listId: string, url: string): Promise<void> {
    try {
      // 1. Check global AI enabled flag
      ...
      // 2. Check wishlist-level AI enabled flag
      const [listRow] = await sql<any[]>`
        SELECT ai_enabled FROM lists WHERE id = ${listId}
      `;
      if (!listRow || !listRow.ai_enabled) {
        console.log(`[AI Review] AI features are disabled for list ${listId}. Skipping extraction.`);
        return;
      }

      // 3. Check if review already exists to avoid duplication
      const [existingReview] = await sql<any[]>`
      ...
      // 4. Fetch the item's name/metadata to provide context to Gemini
      const [itemRow] = await sql<any[]>`
        SELECT name, description, category FROM items WHERE id = ${itemId}
      `;
      if (!itemRow) {
      ...
      console.log(`[AI Review] Triggering review extraction for item: "${itemRow.name}" (${url})`);

      // 5. Extract reviews via Gemini, OpenAI, Anthropic, or Local LLM
      ...
      if (provider !== 'local' && !apiKey) {
        console.log(`[AI Review] No API key configured for ${provider}. Falling back to mock reviews.`);
        reviewData = this.generateMockReviews(itemRow.name, itemRow.category);
      } else {
        reviewData = await this.extractWithAI(
          itemRow.name,
          itemRow.category,
          url,
          provider,
      ...
      await sql`
        ...
        VALUES (
          ${itemId}, 
          ${reviewData.summary}, 
          ${sql.array(reviewData.pros)}, 
          ${sql.array(reviewData.cons)}, 
          ${JSON.stringify(reviewData.reviews)}
        )
      ...
      console.log(`[AI Review] Successfully saved reviews for item: "${itemRow.name}"`);
    } catch (err) {
  ...
  /**
   * Retroactively processes reviews for all items in a wishlist that have links
   * but do not have reviews stored in `item_reviews`.
   */
  static async backfillListReviews(listId: string): Promise<void> {
    try {
      const config = loadConfig();
      if (!config.aiEnabled) return;

      // Find all items on this list that have links but no reviews yet
      const itemsToBackfill = await sql<any[]>`
        SELECT i.id as item_id, il.url 
        FROM items i
      ...
      console.log(`[AI Review] Backfilling reviews for ${itemsToBackfill.length} items in list ${listId}`);

      for (const item of itemsToBackfill) {
        // Run sequentially in background to avoid hitting rate limits
        this.extractAndSaveReviews(item.item_id, listId, item.url).catch(err => {
          console.error(`[AI Review] Failed to backfill review for item ${item.item_id}:`, err);
        });
      }
    } catch (err) {
  ...
  private static compilePrompt(customPrompt: string, itemName: string, category: string, url: string, pageContext: string): string {
    if (!customPrompt) {
      return `
  ...
  private static parseJsonClean(text: string): ReviewData {
    let clean = text.trim();
    if (clean.startsWith('```')) {
  ...
  private static async extractWithAI(
    ...
    let pageContext = '';
    try {
      // Attempt to fetch website to extract title/meta tags for better context
    ...
    if (provider === 'openrouter') {
      ...
      const openrouterUrl = endpoint ? (endpoint.endsWith('/') ? `${endpoint}chat/completions` : `${endpoint}/chat/completions`) : 'https://openrouter.ai/api/v1/chat/completions';
      ...
      const openaiUrl = endpoint ? (endpoint.endsWith('/') ? `${endpoint}chat/completions` : `${endpoint}/chat/completions`) : 'https://api.openai.com/v1/chat/completions';
      ...
      const anthropicUrl = endpoint ? (endpoint.endsWith('/') ? `${endpoint}messages` : `${endpoint}/messages`) : 'https://api.anthropic.com/v1/messages';
      ...
      if (baseEndpoint.includes('localhost:11434') && !baseEndpoint.endsWith('/v1') && !baseEndpoint.endsWith('/v1/')) {
        baseEndpoint = baseEndpoint.endsWith('/') ? `${baseEndpoint}v1` : `${baseEndpoint}/v1`;
      }
      const localUrl = baseEndpoint.endsWith('/') ? `${baseEndpoint}chat/completions` : `${baseEndpoint}/chat/completions`;
      ...
      if (endpoint) {
        geminiUrl = endpoint.endsWith('/') ? `${endpoint}models/${targetModel}:generateContent?key=${apiKey}` : `${endpoint}/models/${targetModel}:generateContent?key=${apiKey}`;
      }
  ...
  private static generateMockReviews(itemName: string, category: string): ReviewData {
    const cleanName = itemName.trim();
    const lowerCategory = (category || 'generic').toLowerCase();

    if (lowerCategory.includes('tech') || cleanName.match(/phone|laptop|ipad|watch|headphone|mouse|keyboard|screen|monitor|charger/i)) {
      return {
    ...
    if (lowerCategory.includes('clothing') || cleanName.match(/shirt|pants|jacket|shoes|dress|socks|hat|coat|sweater|boots/i)) {
      return {
