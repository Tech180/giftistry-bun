...
import { WEBAUTHN_RP_ID_HINT } from '../constants/webauthn-rp-id-hint.constant';
...
export function webAuthnRpIdFromPublicAppUrl(url: string): string {
  ...
  if (!trimmed) {
    throw new AppError(
      `Public App URL is required for passkeys. ${WEBAUTHN_RP_ID_HINT}`,
      400,
      'BAD_REQUEST'
    );
  }
  ...
  try {
    ...
    throw new AppError(
      `Public App URL is invalid. ${WEBAUTHN_RP_ID_HINT}`,
      400,
      'BAD_REQUEST'
    );
  }
  ...
  if (!hostname) {
    throw new AppError(
      `Public App URL is missing a hostname. ${WEBAUTHN_RP_ID_HINT}`,
      400,
      'BAD_REQUEST'
    );
  }
  ...
  if (isIP(host)) {
    throw new AppError(
      `Public App URL hostname "${host}" is an IP address. ${WEBAUTHN_RP_ID_HINT}`,
      400,
      'BAD_REQUEST'
    );
  }
