...
import { APPAREL_SIZE_TOKENS } from '../constants/apparel-size-tokens.constant';
import type { JsonLdProductDetails } from '../interfaces/json-ld-product-details.interface';
import type { JsonLdProductVariant } from '../interfaces/json-ld-product-variant.interface';
...
function readVariant(record: Record<string, unknown>, productName?: string | null): JsonLdProductVariant | null {
  ...
  if (record.offers && typeof record.offers === 'object') {
    ...
    price = parseScrapePrice(offers.price ?? offers.lowPrice ?? offers.highPrice);
  }
