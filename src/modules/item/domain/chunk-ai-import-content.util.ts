...
const DEFAULT_CHUNK_ITEM_LIMIT = 20;
const CHUNK_ITEM_LIMIT_MIN = 1;
const CHUNK_ITEM_LIMIT_MAX = 100;

function resolveChunkOptions(options?: AiImportChunkOptions): {
  enabled: boolean;
  itemLimit: number;
} {
  const enabled = options?.enabled !== false && (options?.enabled ?? DEFAULT_CHUNKING_ENABLED);
  const rawLimit = options?.itemLimit ?? DEFAULT_CHUNK_ITEM_LIMIT;
  const n = typeof rawLimit === 'number' ? rawLimit : Number(rawLimit);
  const itemLimit = Number.isFinite(n)
    ? Math.min(CHUNK_ITEM_LIMIT_MAX, Math.max(CHUNK_ITEM_LIMIT_MIN, Math.round(n)))
    : DEFAULT_CHUNK_ITEM_LIMIT;
