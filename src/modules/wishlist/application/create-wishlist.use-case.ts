...
export class CreateWishlistUseCase {
  ...
  async execute(userId: string, title: string, expiresAtStr?: string, allowGroupFunds: boolean = false): Promise<Wishlist> {
    if (!title) {
    ...
    return await this.wishlistRepo.create(userId, title, expiresAt, allowGroupFunds);
  }
}
