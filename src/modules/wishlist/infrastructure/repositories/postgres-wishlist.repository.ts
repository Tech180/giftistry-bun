...
import type { ListCountsRow } from '../interfaces/list-counts-row.interface';
import { sql } from '@/common/database';
import { mapWishlistOwnerRow } from '../utils/map-wishlist-owner-row.util';
import { mapWishlistWithRoleRow } from '../utils/map-wishlist-with-role-row.util';
import { mapWishlistBaseRow } from '../utils/map-wishlist-base-row.util';
import { mapListShareWithUserRow } from '../utils/map-list-share-with-user-row.util';
import { mapPriorityRow } from '../utils/map-priority-row.util';
...
export class PostgresWishlistRepository implements WishlistRepository {
  async findById(id: string): Promise<Wishlist | null> {
    ...
    return mapWishlistOwnerRow(row);
  }
  ...
  async findByUserId(userId: string): Promise<Wishlist[]> {
    ...
    shares.forEach(share => {
      ...
      listShares.push(mapListShareWithUserRow(share));
      sharesByListId.set(share.ListId, listShares);
    });

    return rows.map(row => ({
      ...mapWishlistWithRoleRow(row),
  ...
  async findExpiredActive(): Promise<Wishlist[]> {
    ...
    `;
    return rows.map(mapWishlistBaseRow);
  }
  ...
  async findPrioritiesByWishlistForUser(wishlistId: string, userId: string, isOwner: boolean): Promise<Priority[]> {
    ...
    const allPriorities = [...ownedPriorities, ...rows.map(mapPriorityRow)];
